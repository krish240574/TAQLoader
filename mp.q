...
fpth:{show "fpth";` sv 'dirs[x`part],/:(`$string raze x`date),\:`QUOTE,`}
...
gd:{show x;show y;show z;delete part from select from x where part=y,date=dt[z]}
/ write to disk with enum -x-chunk,y-partition from gp,z-path-`:/data/7/2000.01.01/QUOTE/
wr:{show z;show y;pth[z] set .Q.en[pth[z]] gd[x;y;z]}
...
foo:{
 ...
 {show y;p:fpth[y];wr[x;y`part]each p;}[x]each 0!t}
